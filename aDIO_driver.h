/**
    ...
    $Id: aDIO_driver.h 37447 2009-05-07 21:13:07Z wtate $
*/
 ...
 */
#ifdef __INTEL_MONTAVINA_CHIPSET
    #define ADIO_SETUP_PORT 0x9CF
#elif __INTEL_855GME_CHIPSET
    #define ADIO_SETUP_PORT 0x45F
#elif __GEODELX_CHIPSET
    #define ADIO_SETUP_PORT 0x45F
#else
    #define ADIO_SETUP_PORT 0x1F
#endif

#define MULTIPORT_MODE_MASK_MONTAVINA 0x88
#define MULTIPORT_MODE_MONTAVINA 0x80
...
INTERRUPT_HANDLER_TYPE adio_interrupt(int irq,
				      board_t * dev, struct pt_regs *regs);
#else
INTERRUPT_HANDLER_TYPE adio_interrupt(int irq, board_t * dev);
#endif
...
/******************************************************************************
@brief
    This function allows the Makefile to see ADIO_MAX_BOARDS so that it
    knows how many device nodes to make in the device directory

@param
    buffer
    
    character pointer where the data is written for output
    
@param
    start
    
    signifies where data is stored in this buffer (not used)
    
@param
    offset
    
    Address of memory containing the offset within the file
    being accessed.  Most read operations would update this
    offset, e.g. when reading from a normal file.  However,
    this offset has no meaning for a aDIO device and it is
    not updated. (not used)
...
@param
    ...
    output flag signifying that the end of file has been reached
    
@param
    data
    
    can be used for internal book keeping (not used)
 ...
 ******************************************************************************/
static int
aDIO_read_proc_entry(char *buffer,
		     char **start,
...
aDIO_init_module()
    ...
    Purpose:
        ...
        1)  It looks at the value of minor_num_to_auto_detect and if the number
        is between 0 and ADIO_MAX_BOARDS-1 the onboard(CPU board) aDIO is
        given this minor number and retrieves its io address and irq number
        from a register provided in BIOS
        2)  It reads in the io array and irq array from the insmod command line
        and gives these the remaining minor numbers in order.
        3)  Registers the character device with the kernel
        4)  initializes lock, counter, flags, and wait_que for each device and
